...
router.post("/reg",cors(), async (req, res) => {
  ...
  try {
    ...
    const unuser = new Usr({ firstName, lastName, emailId, password });
    await unuser.save();
...
//   if (!queryName || !optionName || !value || !totalVotes) {
//     return res.status(422).json({ error: "Please entered every field." });
//   }
